...
| `.github/tee/secretvm.env` | Pins SecretVM release version (currently `v0.0.31` — the portal's Production "Artifacts version"; GitHub still labels it pre-release), rootfs variant (`rootfs-prod`), **TDX rootfs URL/SHA-256 AND SEV rootfs URL/SHA-256, plus the SEV artifact registry URL and registry vm_type**. All pipeline rootfs references derive from this file. |
