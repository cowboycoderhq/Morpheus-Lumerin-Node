...
Each slice, and the whole diff twice, was reviewed against pre-registered
falsifiers. Findings were fixed before merge; final verdicts:

- Money surface (send): confirm flow safe, guard correct.
...
- Whole diff (final): cross-slice coherent, no stray files, typecheck green.
