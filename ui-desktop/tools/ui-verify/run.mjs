...
// The bytes32 used by the close-session cases. See that file for how it
// is derived and why it resolves to no session.
const SESSION_ID = '0x039235b5abdc97bc4d2fdf5fb9c4ba23d1cd7d93445ca20ca13690e33ef57f0a';
...
// --- sendform: two-step confirm; onSubmit must NOT fire on the first press ---
{
  ...
  await drive(page, 'sendform-confirm', `http://localhost:${PORT}/?case=sendform`, async (p) => {
    ...
    assert(panel.includes('0x189971F43CD98F6c837ca1b74020ed370e2104D4'), 'confirm panel missing full toAddress');
    assert(
      !panel.includes('0x462198870c58a805643de4d1EbF305135F3dF071'),
...
//     swap re-renders tokens live, persists + rehydrates ---
{
  ...
  await drive(page, 'theme-swap', `http://localhost:${PORT}/?case=theme-swap`, async (p) => {
    ...
    await p.evaluate(() => window.localStorage.removeItem('morpheus.themeVariant'));
    ...
    const stored = await p.evaluate(() => window.localStorage.getItem('morpheus.themeVariant'));
...
//     screen when you agree to it ---
{
  ...
  await drive(page, 'terms-two-consents', `http://localhost:${PORT}/?case=terms`, async (p) => {
    ...
    // The agreement is IN FRONT of the user, not behind a link. Replacing this
    // scroll box with a one-line summary would not be a re-skin.
...
// --- shell: the rail renders, Help keeps its onHelpLinkClick contract, and
//     the rail's own surface re-renders on a theme swap (not just a probe) ---
{
  ...
  await drive(page, 'shell-sidebar', `http://localhost:${PORT}/?case=shell`, async (p) => {
    ...
    // Help offers a choice instead of guessing. Opening
...
//     and it lives in the one file the restyling fully rewrites. If a future
...
{
  const page = await browser.newPage({ viewport: { width: 900, height: 900 } });
  // The spend caps that used to be tested here are gone: nothing but this app
  // can open a session, so there was no unattended spending left to bound, and
  // as settings they were three more boxes between "endpoint on" and "it
  // works". The case went with them rather than rotting against fields that no
  // longer exist.
...
// stake(T) = T x 1e15 wei. 305s -> 0.305 MOR, 86,400s -> 86.40, 604,800s -> 604.80.
{
  ...
  // dropped and the plan is STILL one block. A later pass caught the UI asking
...
// Fixture is chain-derived and resolves to no session; see close-session.jsx.
// The tab has to be opened first — sessions live behind it.
{
  ...
  await drive(page, 'close-session-warns', `http://localhost:${PORT}/?case=close-session&at=1787872961`, async (p) => {
    ...
    // 306s of a 1199s session on a 229.508381 MOR stake -> 58.5734 locked / 170.9349 back.
    const panel = await p.locator('[data-testid="close-session-confirm"]').innerText();
    assert(/58\.5734 MOR/.test(panel), `confirm did not state the locked amount: ${panel}`);
    assert(/170\.9349 MOR/.test(panel), `confirm did not state what comes back: ${panel}`);
  ...
  await drive(page, 'close-session-ended-shows-no-lock-warning', `http://localhost:${PORT}/?case=close-session&at=1787873854`, async (p) => {
    await p.waitForSelector('text=Sessions', { timeout: 20000 });
    await p.getByText('Sessions', { exact: false }).first().click();
...
// The review's structural criticism of the other session-length cases:
