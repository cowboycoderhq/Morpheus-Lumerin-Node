...
console.log('queries: earlyCloseLock (the Close button warning)');
{
  ...
  // their full duration and closed a few seconds LATE returned 0.06%
  // and pushed the rest onto userStakesOnHold, matched to the wei
  ...
  ok('closing AT endsAt locks the WHOLE stake', atEnd.known && atEnd.lockedWei === BigInt(FIXTURE.Stake));
  ...
  ok('closing AFTER endsAt still locks the whole stake', after.lockedWei === BigInt(FIXTURE.Stake));
  ...
  // on one UTC day and were closed on the next returned their stake IN FULL,
  // hold delta exactly 0. They also did NOT revert,
  // which rules out the underflow this expression would suffer if the deployed
  // contract were the vendored one minus its guard.
  const nextDay = earlyCloseLock(FIXTURE, 1787875200 + 483); // past the next UTC midnight
  ...
  ok('a close on the next UTC day refunds in full',
    nextDay.returnedWei === BigInt(FIXTURE.Stake));
  ok('the same-day/next-day split is real, not a rounding artefact',
    atEnd.lockedWei === BigInt(FIXTURE.Stake) && nextDay.lockedWei === 0n);
...
console.log('queries: nextStakeReleaseAt (the On Hold tile clock)');
{
  ...
  ok('closed at endsAt -> releases at startOfDay(close) + 1 day',
    nextStakeReleaseAt([late], 1787873407) === 1787875200);
  const later = { ...FIXTURE, ClosedAt: 1787874165 };
  ok('closed after endsAt -> still a release date',
    nextStakeReleaseAt([later], 1787873407) === 1787875200);
...
// again when review found the last block was always full-size, so an
...
console.log('queries: blocksForDuration vs the scheduler stop condition');
{
  ...
  // full-size-final-block behaviour that review found was costing
...
console.log('queries: block accounting at the 7-day cap unit');
{
  ...
  // 2 years is the worked example. It must be a finite, sane plan.
...
// Defect (HIGH): the final block used to be a full cap-sized
...
console.log('queries: a chained run stops at the length that was sold');
{
  ...
  // whole days at the cap. A later pass caught the earlier version overclaiming
  ...
  // The arithmetic behind a UI defect review found: just past the cap the
  ...
  // The two cases measured, by name.
...
{
  // Hand-written for this test and 32 hex long, where generateToken()
  // emits randomBytes(24) — 48 hex — so it cannot be a production
  // token. It is passed only to admitRequest() and bearerMatches(), both
  // pure: no I/O, no network, no storage. admitRequest() additionally
  // refuses any non-loopback host. The literal appears in no other file.
...
console.log('grok: models published into the managed config');
{
  ...
  // the only three things they need: was I charged, whose fault, what now.
  {
    const providerDead =
      ...
      'error: failed to decode response: read tcp 10.0.0.2:59378->203.0.113.42:3333: ' +
  ...
  // they are asserted at the seam. Each one is a defect that shipped.
  {
  ...
  // the actual defect — the deepseek names were only how it surfaced.
  {
    ...
    const impostor = [
      M('0xdf33f889aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'foo'),
      M('0x9999999999999999999999999999999999999999999999999999999999999999', 'foo#df33f889'),
    ];
    const r = resolveModel('foo#df33f889', impostor);
    ok('a name posing as a discriminator never resolves to the OTHER model',
      !r.ok || r.model.name === 'foo#df33f889');
...
console.log('duration: parsing a typed session length');
{
  ...
  // and they must come back to the input exactly, for every value.
  {
    ...
    for (const sec of [
      ...
      // 1.983 years and 3y1mo16d — the two measured as lossy.
...
// wallet with plenty spare, and the error read "3.0677580977e+21 MOR".
...
console.log('queries: the reserve counts every live run');
{
  ...
  // MOR rendered "3.0677580977e+21 MOR".
  const OBSERVED_WEI = 3.0677580977e21;
