...
const IconBadge = styled.div`
  ...
  background: ${(p) => p.theme.colors.warningTint(0.12)};
...
const redactDiagnostics = (s: string): string =>
  s
    ...
    // 12-24 short lowercase words separated by whitespace, commas, or quotes —
    // covers plain, newline, comma, and JSON-array mnemonic forms.
    .replace(/\b([a-z]{3,8}[\s"',]+){11,23}[a-z]{3,8}\b/g, '[REDACTED_MNEMONIC]')
    // any bearer/basic Authorization header
