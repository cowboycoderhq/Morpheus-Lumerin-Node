...
// THINKING state: the ring sweeps. An arc, not a full ring,
...
export const SessionLengthInput = styled.input.attrs({
  ...
  /* Plain high-contrast text, NOT the brand accent. The accent is a saturated
     cyan under Lumen; on text the user is actively typing and re-reading it is
...
// The voice orb, at rest.
//
// The orb is a ring that breathes with the live mic/playback level,
...
export const ModelGlyph = styled.div<{ $thinking?: boolean }>`
  ...
  /* inner ring — and, while the model is thinking, a sweeping arc: the ring
     spins the way the voice orb's does while a response streams. */
  &::after {
    content: '';
    position: absolute;
    inset: 0.55rem;
