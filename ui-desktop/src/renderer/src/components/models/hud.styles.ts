...
//
// It is built from the app's OWN tokens rather than JARVIS's raw hex. JARVIS's
// accent (#6fd6ff) maps to `secondaryLight` (#3fe0ff), which the Aurora theme
// already designates for "info / data" — so the page reads as the same product,
// not a second one bolted on. Brand green is kept for one thing only: live/OK
// status, the same meaning it carries everywhere else in the app.
// ============================================================================
...
export const HudHeader = styled.header`
  ...
  border-bottom: 1px solid rgba(94, 208, 255, 0.26);
`;
...
export const HudTitle = styled.h1`
  ...
  text-shadow: 0 0 18px rgba(94, 208, 255, 0.45);
`;
...
export const Orb = styled.span<{ $on: boolean }>`
  ...
  box-shadow: ${(p) =>
    p.$on
      ? '0 0 10px rgba(89, 227, 167, 0.9)'
      : '0 0 10px rgba(255, 92, 106, 0.9)'};
...
export const HudBtn = styled.button`
  ...
  border: 1px solid rgba(94, 208, 255, 0.5);
  border-radius: ${(p) => p.theme.radii.md};
  transition:
  ...
  &:hover {
    background: rgba(94, 208, 255, 0.17);
...
export const HudPanel = styled.section`
  ...
  border: 1px solid rgba(94, 208, 255, 0.26);
...
export const HudTabsWrap = styled.div`
  ...
  #tab-models {
    ...
    .nav-link.active {
      ...
      box-shadow: 0 0 16px rgba(94, 208, 255, 0.18);
    }
  }
`;
