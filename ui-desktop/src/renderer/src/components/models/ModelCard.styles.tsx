...
export const Card = styled.div`
  ...
  /* Corner bracket — a HUD reticule mark rather than a decorative flourish:
     it marks the panel's origin the way a terminal HUD frames its data. */
  &::before {
    content: '';
    position: absolute;
    top: 0;
  ...
  &:hover {
    ...
    border-color: ${(p) => p.theme.colors.brandTint(0.55)};
    transform: translateY(-2px);
    box-shadow: 0 0 20px ${(p) => p.theme.colors.brandTint(0.14)};
  }
...
export const CardTitle = styled.div`
  ...
  text-shadow: 0 0 12px ${(p) => p.theme.colors.brandTint(0.35)};
`;
...
export const IconButton = styled.button<{ $danger?: boolean }>`
  ...
  &:focus-visible {
    background: ${(p) =>
      p.$danger ? p.theme.colors.dangerTint(0.16) : p.theme.colors.brandTint(0.15)};
...
// everywhere it appears. No dedicated theme token exists for that hue family,
// so it borrows the brand-bright pairing (still fully theme-driven) rather
...
export const Spinner = styled.span`
  ...
  width: 1.4rem;
  height: 1.4rem;
  border: 2px solid ${(p) => p.theme.colors.brandTint(0.25)};
