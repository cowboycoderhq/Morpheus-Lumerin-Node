...
const CalloutBox = styled(Flex.Row)<{ $tone: CalloutTone }>`
  ...
  background: ${(p) =>
    p.$tone === 'warning' ? 'rgba(255, 92, 106, 0.08)' : p.theme.colors.glassSurface};
  border: 1px solid
    ${(p) =>
      p.$tone === 'warning'
        ? 'rgba(255, 92, 106, 0.3)'
...
export const DangerBtn = styled(Btn)`
  ...
  background-color: rgba(224, 92, 115, 0.1);
  border: 1px solid rgba(224, 92, 115, 0.5);

  &:not([disabled], [data-disabled]):hover,
  &:not([disabled], [data-disabled]):focus {
    color: ${(p) => p.theme.colors.danger};
    background-color: rgba(224, 92, 115, 0.2);
    box-shadow: 0 0 16px rgba(224, 92, 115, 0.25);
  }
`;
