...
// file: PR #830 (214024e0, 2026-07-29) rewrote _rewardUserAfterClose to anchor
...
//
// Measured on Base mainnet 2026-08-06 (public sessions):
// two sessions of identical stake, each run to its full duration and closed
// a few seconds AFTER endsAt — a late close under any reading — returned
// 0.06% to the user and pushed the rest onto userStakesOnHold. The hold
// delta matched the withheld amount to the wei at the closing block, on both.
// Four more late closes showed the same 0.05–0.08%.
//
...
// remaining stake comes back in the closing tx. The two mainnet closes described
// above landed seconds after endsAt — still inside the session's own UTC day,
// which is why they locked ~everything.
// A real user closed a session well before it ended and watched part of the stake
// disappear for a day with no warning at all.
//
...
// a conservative ceiling, never an under-promise. The formula was checked against real on-chain
// early closes before it was relied on; what the fixture pins is what it
// predicts from chain-derived inputs.
//
