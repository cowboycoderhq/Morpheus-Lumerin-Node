...
<Note>
  **In a hurry? Pick the fastest path.**
  ...
  - **Try it free — no wallet, no tokens:** after install, the app also downloads a local `llama.cpp`
    demo model so you can chat without staking MOR. In Chat, open **New chat** — it is the model under the **Local** section.
...
<CardGroup cols={2}>
  ...
  <Card title="llama.cpp + Qwen2.5-1.5B" icon="microchip">
