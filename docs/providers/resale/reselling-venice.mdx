...
<Steps>
  ...
  <Step title="Price above your Venice cost">
    See [Pricing a resale bid](/providers/resale/registering-bid). Every bid costs a non-refundable `marketplaceBidFee` (`0.3` MOR on Base mainnet; `0` on Base Sepolia, where bids are free) — finish config before posting.
  </Step>
  <Step title="Verify">
