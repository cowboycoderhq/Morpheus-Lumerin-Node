...
4. Consumer prompts arrive on port `:3333` at your proxy-router, which handles the public consumer-to-provider session and inference; port `:8082` is for private/admin API and Swagger. The proxy-router forwards to the upstream `apiUrl`. Upstream responses stream back the same way.
