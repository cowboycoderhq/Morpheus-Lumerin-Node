...
If you used [MyProvider](/providers/full/myprovider-gui), confirm the on-chain `modelId` is in `models-config.json` (edit the file directly if needed) before Step 1.
...
```json
{ "status": "healthy", "version": "<current-version>", "uptime": "..." }
```

On provider nodes running **v7.4.0 or later** the response also includes a `models` array — the periodic model health self-report covering your **whole ecosystem of configured models and active bids**. The body shown above is the short shape: a node predating v7.4.0 returns no `models` key at all, and its absence there is not a fault. For every model in `models-config.json` that has an active bid from your wallet, the proxy-router sends a small test prompt on a schedule (default hourly, see `MODEL_HEALTH_CHECK_INTERVAL` in [env-proxy-router](/reference/env-proxy-router)) and caches the result. Probe pacing (`MODEL_HEALTH_CHECK_PROBE_DELAY`, default `2s`, so a provider with many models doesn't burst its own backend into a rate limit) arrived in the release **after** v7.4.0 — on a v7.4.0 node you get the report without it. Active bids whose model is *missing* from `models-config.json` are also listed, so a bid you're selling but cannot serve doesn't go unnoticed. [Model health](/providers/full/model-health) is the full reference for this array:
...
| `degraded` | Probe hit a rate limit — backend is throttled | Review rate-limit settings; consumers may get 429 errors |
...
| `tee_unverified` | `tee`-tagged model whose backend TEE attestation failed or never succeeded; **not probed**, and session requests for it are rejected | Fix the backend attestation — consumers cannot open a session against this model while it reports `tee_unverified` |

<Note>
Version floors inside this report: `modelName` and `httpStatus`, the `rate_limited` error kind, and `MODEL_HEALTH_CHECK_PROBE_DELAY` arrived in **v7.5.0**; the `degraded` status arrived in **v7.9.0** (`proxy-router/internal/system/structs.go`). A v7.4.0 node carries neither `modelName` nor `httpStatus`, and a v7.5.0–v7.8.x node reports a rate-limited backend as `unhealthy` with `errorKind: rate_limited` rather than `degraded`.
</Note>
...
For `unhealthy` models, `errorKind` narrows the failure (except `rate_limited`, which results in a `degraded` status) and, when the backend answered with an HTTP error, `httpStatus` carries the exact upstream status code:
...
| `rate_limited` | Backend is up and configured, but throttled the probe (results in `degraded` status) | `429` |
...
2. The dashboard's **API Test** page dropdown only contains the bootstrap set; independently registered models will not appear there. Instead, test via a self-hosted proxy-router or direct API calls (see [API direct](/reference/api-direct)).
3. Send any short prompt.

If it streams back a response — **you're done**. The dropdown only contains the bootstrap set; independently registered models will not appear there. Providers should test via a self-hosted proxy-router or direct API calls instead.
...
| Lost ~MOR during setup | Each `postModelBid` charges a non-refundable `marketplaceBidFee` (`0.3` MOR on Base mainnet — live on-chain; `0` on Base Sepolia, where bids are free); reposting bids during configuration adds up | See [Pricing](/providers/full/pricing) and the "What can cost you MOR during setup" section in [Quickstart](/providers/full/quickstart) |
