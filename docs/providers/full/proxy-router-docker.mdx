...
| `.cookie` | Admin credentials for the local API — taken from `COOKIE_CONTENT` if set, otherwise generated randomly |
...
Either way the credential is written to the `COOKIE_FILE_PATH` set below,
`/app/data/.cookie`, with `0600` permissions. That path is inside the container,
so **read the generated password from the host mount**. With the
`-v /path/to/local/data:/app/data` bind used here, that is
`cat /path/to/local/data/.cookie`. The file holds a single `admin:<password>`
line, and that is what you enter in Swagger's **Authorize** box.

The file is only written when it does **not** already exist (`auth.go:215`), so
if you have started the container before, delete it first or the change has no
effect.
</Warning>
...
# then remove COOKIE_CONTENT=admin:admin from .env — see the warning above
vi .env
