...
<Note>
The `httpStatus` and `modelName` report fields, the `rate_limited` error kind, probe pacing (`MODEL_HEALTH_CHECK_PROBE_DELAY`), and the manual `POST /healthcheck/models/refresh` trigger arrived in **v7.5.0** — the release after v7.4.0. On v7.4.0 nodes you get the report without those features. The `degraded` status (rate-limited backends) and the consumer-side `SESSION_HEALTH_POLICY` arrived later still, in **v7.9.0**; on v7.5.0–v7.8.x nodes a rate-limited backend reports `unhealthy` with `errorKind: rate_limited`.
</Note>
...
| `MODEL_HEALTH_MAX_CONSECUTIVE_ERRORS` | `3` | Consecutive real-session prompt failures after which a model is flipped to `degraded` when every failure in the streak was a 429 (upstream rate limiting), and `unhealthy` otherwise, with `errorKind: session_errors` in both cases, immediately, without waiting for the next sweep |
