...
| Registry URL | `https://cdn.jsdelivr.net/npm/secretvm-verify@0.13.0/data/tdx.csv` | `https://cdn.jsdelivr.net/npm/secretvm-verify@0.13.0/data/sev.json` |
...
sequenceDiagram
    ...
    Note over P: For each entry:<br/>Build cmdline with compose hash<br/>Compute GCTX launch digest<br/>Compare vs quote measurement<br/>No loaded SEV registry: attestation fails
...
Like TDX, this step is mandatory. If the SEV registry is `nil` or was never loaded, `VerifyWorkload` returns `artifact_registry_not_available` and attestation fails rather than passing unverified (`workload_verifier.go:106-110`, `backend_verifier.go:211-213`). An unfetchable `/docker-compose` is fatal for the same reason (`backend_verifier.go:196-199`). Fail-closed since v7.4.0.
...
|----------|---------|-------------|
| `SEV_ARTIFACT_REGISTRY_URL` | `https://cdn.jsdelivr.net/npm/secretvm-verify@0.13.0/data/sev.json` | URL for the SEV-SNP artifact registry JSON ([secretvm-verify](https://www.npmjs.com/package/secretvm-verify)) |
| `ARTIFACT_REGISTRY_REFRESH_INTERVAL` | `1h` | Shared refresh interval for both TDX and SEV registries; if unset or set to `0`, the constructor falls back to `1h` (`artifacts_registry.go:51-53`) |
