...
The compose includes a Traefik TLS sidecar that uses SecretVM's auto-generated certs to serve the API on HTTPS port 443. `/healthcheck` is how you check **your own** node, and Step 4 below does exactly that. What it is not is a way to check **someone else's** node: `/healthcheck` lives on the `:8082` admin API, which a non-TEE provider should not publish — to read another provider's self-report, ask a C-Node to relay it ([model-health.mdx](/providers/full/model-health)). The full compose body is reproduced in [TEE reference → Step 1](/providers/full/tee-reference#step-1-prepare-the-docker-compose-file).
...
If Step 0 already gave you an existing on-chain `Id`, put that value in `modelId` now. If you will mint a new `tee` model after deploy, use a placeholder and later edit `models-config.json` directly and restart, or delete the file and set `MODELS_CONFIG_CONTENT` again before posting a bid you care about.
...
<Tabs>
  <Tab title="Web portal">
    ...
    |---------|-------------|-------|
| **Platform** | Intel TDX | Both platforms have CI/CD golden values; Intel TDX is recommended |
...
<Steps>
  ...
  <Step title="Sync models-config">
    For persistent SecretVM deployments, `MODELS_CONFIG_CONTENT` only seeds the `models-config.json` file on the first run. After the file exists, changing the environment variable has no effect. To update models, edit the `models-config.json` file directly (e.g., via SecretVM file injection) and restart the proxy-router.
  </Step>
</Steps>
