...
<Steps>
  ...
  <Step title="Validate the model is reachable">
    From the proxy-router instance: `curl -X POST http://<model_host>:<model_port>/v1/chat/completions -d '{...}'` should respond — that endpoint is POST-only, so a plain `curl` (which sends GET) proves nothing. That is the only path the Part 1 security group admits — inbound `8080` is open to the proxy-router instance's SG and to nothing else, so a browser on your own machine pointed at `http://<model_host>:<model_port>` times out by design, not by fault. To see the llama.cpp UI, forward the port through an SSH session to the proxy-router instance first:

    ```bash
    ssh -L 8080:<model-instance-private-dns>:8080 ec2-user@<proxy-router-public-dns>
    ```

    then open `http://localhost:8080` on your machine. Do not widen the security group to make a browser check pass.
  </Step>
</Steps>
