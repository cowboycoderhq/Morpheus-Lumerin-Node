...
- Blockchain config (contract addresses, chain ID, blockscout URL) is pinned to the target network (`proxy-router/Dockerfile.tee:13-16`, surfaced in the signed manifest's `baked_env`).
- Chat context storage is disabled (`PROXY_STORE_CHAT_CONTEXT=false`, `proxy-router/Dockerfile.tee:23`, surfaced in the signed manifest's `baked_env`) — but that `baked_env` field is a workflow literal (`.github/workflows/build.yml:957`), so unlike the logging pins it is not read back out of the image.
- Logging is set to production mode: `LOG_JSON=true`, `LOG_COLOR=false`, `LOG_IS_PROD=true`, with the levels pinned at `LOG_LEVEL_APP=info` and `LOG_LEVEL_TCP`/`LOG_LEVEL_ETH_RPC`/`LOG_LEVEL_STORAGE=warn` (`proxy-router/Dockerfile.tee:30-36`, surfaced in the signed manifest's `baked_env`). That is quieter than the compiled `debug` app default, not the quietest level — the publish job refuses to ship a manifest if `LOG_LEVEL_APP=debug` is baked in (`.github/workflows/build.yml:820-822`).
- Environment is pinned to `production` (`proxy-router/Dockerfile.tee:19`, surfaced in the signed manifest's `baked_env`).
...
The deployment injects 5 per-provider secrets, and the signed manifest names exactly these in `runtime_secrets_only` (`.github/workflows/build.yml:944-950`). That field is a **declaration, not an enforced boundary** — see **What the attestation actually pins** below:
...
| `WEB_PUBLIC_URL` | Public-facing URL for the API (default: `https://localhost`) |
| `COOKIE_CONTENT` | API auth credentials (default: `admin:admin`) |

**What the attestation actually pins.** RTMR3 is the replay of two inputs — the compose bytes and the rootfs digest — and nothing else (`proxy-router/internal/attestation/rtmr.go:12`), over a compose that names the image by immutable digest. So editing the compose, including adding an `environment:` entry, changes the measurement and the consumer's replay fails. It does **not** pin the running container's environment. The compose declares `env_file: - usr/.env` (`proxy-router/docker-compose.tee.yml:15-16`), the file SecretVM fills from your encrypted-secrets input; the reference is inside the measured bytes, the file's contents are not, and Docker precedence puts `env_file` above the image's `ENV`. Any baked value the compose does not also name in `environment:` — chat-context storage, every `LOG_*`, `ENVIRONMENT`, the blockchain settings — can be set from that file with RTMR3 unchanged. Nothing downstream re-forces it: the loader reads every `env:`-tagged field straight from the process environment with no allowlist (`proxy-router/internal/config/loader.go:54-61`), the entrypoint is a bare `proxy-router` (`proxy-router/Dockerfile.tee:45`), and `baked_env` is parsed into a `json.RawMessage` no code reads (`proxy-router/internal/attestation/golden.go:48`). Read `baked_env` as what the image was **built** with, not as what the container is **running**.
...
- An RPC endpoint for the target network (e.g. `https://base-mainnet.g.alchemy.com/v2/<your_key>`).
...
**Exact byte content matters.** The compose file must contain the full content as shown in the example, including the `networks:` and `configs:` sections, because RTMR3 is computed from the entire file. RTMR3 is computed from the exact byte content — a single extra or missing byte changes the hash. The compose file includes the `networks:` and `configs:` sections after `proxy_data: null`, so the entire file content is used for RTMR3 computation.
</Warning>
...
As a consumer you can independently verify a provider before using it. Automatic verification is already built into the C-Node (v6.0.0+): the consumer's proxy-router fetches the provider's attestation, verifies it against the golden values published in the TEE image's attestation manifest (using cosign), checks TLS binding, compares RTMR3, and re-checks on every prompt.
...
| `runtime_secrets_only` | The 5 per-provider secrets the deployment is expected to inject — a declaration, not an enforced boundary |
...
| Phase 2 backend developer reference | [TEE backend verification](/providers/full/tee-backend-verification) |
