...
<Steps>
  ...
  <Step title="Done">
    Spendable MOR is back in your wallet — everything except the final UTC day's locked slice lands at close, and that slice after the day-lock releases + claim. It is not an "unused now, used later" split: on a multi-day session the earlier days' consumed stake is in the amount returned at close.
  </Step>
</Steps>
...
If a session is genuinely stuck (e.g. pool-mode funding account empty, your consumer node offline past `endsAt`), the resolution is still a successful `closeSession` followed, if needed, by `withdrawUserStakes`. There is no other path.
