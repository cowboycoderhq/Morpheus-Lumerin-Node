...
<Steps>
  ...
  <Step title="The day-locked slice comes back — while your node is running">
    After `releaseAt` (≈ after the end of the UTC day the session ended on) a running proxy-router sweeps it automatically every 10 minutes, and only for the wallet it holds. With the node off, or for a session opened from another wallet, nothing sweeps and you must call `withdrawUserStakes(yourAddress, iterations)` on the Diamond contract yourself; with that node running you call it only to take the withdrawal sooner. **There is no HTTP route for this on the proxy-router today** — use `cast send`, MetaMask "Interact with contract", or your wallet's contract UI.
