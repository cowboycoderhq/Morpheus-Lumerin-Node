...
description: "The complete lifecycle of a Morpheus session — open, stake, close, day-lock of the final UTC day's consumption, and the sweep back by the proxy-router's StakeClaimer, which runs only while your own node is running and only for the wallet it holds; otherwise nothing sweeps until a proxy-router holding that wallet runs — starting one claims it immediately on startup — and the manual withdrawUserStakes call is the alternative. Why MOR is not 'gone' while a session is open, and why only the final day's consumption is locked."
...
flowchart TB
  ...
  Hold --> Claim["6. StakeClaimer auto-sweep after releaseAt — only while your own node runs and only for the wallet it holds; with that node off or the stake held against a different wallet, nothing sweeps until a proxy-router holding that wallet runs — starting one claims it immediately on startup, or call withdrawUserStakes yourself"]
...
`userStakesOnHold[user]` array. Holds `min(your remaining stake, the stake-equivalent of the seconds consumed during the **final UTC day**)` until `releaseAt = startOfTheDay(min(closedAt, endsAt)) + 1 day` — but only when the close lands **before** `releaseAt` (`SessionRouter.sol:305`). A later close skips the lock entirely and the whole remaining stake returns at close, matching the walkthrough's "if you close before that timestamp" below. Cleared automatically by the proxy-router's StakeClaimer — but only while your own node is running and only for the wallet it holds, so with the node off, or for a session opened from another wallet, nothing sweeps until a proxy-router holding that wallet runs: starting one claims matured stake immediately on startup, and a manual `withdrawUserStakes` is the alternative.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="The day-locked slice comes back — while your node is running">
    After `releaseAt` (≈ after the end of the UTC day the session ended on) a running proxy-router sweeps it automatically every 10 minutes, and only for the wallet it holds. With the node off, or for a session opened from another wallet, nothing sweeps until a proxy-router holding that wallet runs — starting one claims matured stake immediately on startup — and calling `withdrawUserStakes(yourAddress, iterations)` on the Diamond contract yourself is the alternative; with that node running you call it only to take the withdrawal sooner. **There is no HTTP route for this on the proxy-router today** — use `cast send`, MetaMask "Interact with contract", or your wallet's contract UI.
  ...
  <Step title="Done">
    Spendable MOR is back in your wallet — everything except the final UTC day's locked slice lands at close, and that slice once the day-lock expires and the proxy-router's StakeClaimer sweeps it back, which needs no manual claim while your own node is running and holds the wallet you opened from. With the node off, or for a session opened from another wallet, nothing sweeps until a proxy-router holding that wallet runs — starting one claims matured stake immediately on startup — and submitting `withdrawUserStakes` yourself is the alternative, never the only route. It is not an "unused now, used later" split: on a multi-day session the earlier days' consumed stake is in the amount returned at close.
  </Step>
</Steps>
...
- Opening many short sessions that each "use" most of their stake will park that capital until the next UTC day, when a running proxy-router sweeps it back automatically for the wallet it holds. The wait is the day-lock, not a manual claim step — unless the node is off or the session was opened from another wallet, in which case nothing sweeps until a proxy-router holding that wallet runs — starting one claims it immediately on startup — and the manual claim is the alternative.
...
stateDiagram-v2
  ...
  Closed --> OnHold: final-day slice parked until releaseAt_ = startOfTheDay(min(closedAt, endsAt)) + 1 day, and only if the close lands before releaseAt_
  ...
  OnHold --> Swept: withdrawUserStakes mined once releaseAt_ has passed — releasable rows popped and safeTransferred in the same call — by your node's StakeClaimer (polls every 10 min) or by you
  Swept --> [*]: stake is back in your wallet, nothing left on hold
```

`withdrawUserStakes` pops the releasable rows and `safeTransfer`s them in a single call (`SessionRouter.sol:448-465`), so there is no on-chain step between `OnHold` and `Swept`: a submitted-but-unmined withdrawal has changed nothing, and one that reverts (`SessionUserAmountToWithdrawIsZero`, `:444` and `:462`) leaves the queue exactly as it was — which is the self-loop above. The same six states and edges are listed with their side effects in [Session states (open, close, claim)](/ai/session-states-open-close-recover), the agent-citable version of this diagram.
...
If a session is genuinely stuck (e.g. pool-mode funding account empty, your consumer node offline past `endsAt`), the resolution is still a successful `closeSession`. The follow-up `withdrawUserStakes` is submitted for you by the proxy-router's StakeClaimer once the day-lock matures — but only if that node is yours and running, and only for the wallet it holds. The StakeClaimer is started inside `Proxy.Run` (`proxy-router/internal/proxyctl/proxyctl.go:237-240`) and withdraws for `GetMyAddress` alone (`proxy-router/internal/blockchainapi/service.go:1118-1124`), so in exactly the case this paragraph describes — a consumer node offline past `endsAt` — nothing sweeps until a proxy-router holding that wallet runs: bringing that node back up claims the matured stake immediately on startup (`stake_claimer.go:87-89`), and submitting `withdrawUserStakes` yourself as well as the close is the alternative. With the node running you call it only to take the withdrawal sooner. `withdrawUserStakes` is the only on-chain function that moves the money either way — the two routes differ only in who submits it.
