...
<CardGroup cols={2}>
  <Card title="Provider AI Model" icon="brain">
    Any HTTP endpoint compatible with the configured `apiType` (OpenAI, claudeai, prodia-v2, hyperbolic-sd, etc.). Reachable privately by the provider's proxy-router.
  </Card>
...
sequenceDiagram
  ...
  PRouter->>Model: Forwarded request (format per apiType)
...
| `21434` (HTTPS, host-net Caddy) and `29343` (HTTPS, attest-rest) | TEE-only — SecretVM | Public | TDX attestation (`/cpu`, `/gpu`, `/docker-compose`) |
