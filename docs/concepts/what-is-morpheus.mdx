...
<CardGroup cols={3}>
  ...
  <Card title="Session-time pricing, not tokens" icon="hourglass">
    Pricing is **per-second of session time**, not per-token. Consumers stake MOR for a fixed duration; the MOR is escrowed on open. On close, the final UTC day's consumed slice day-locks until the next UTC day and the **unconsumed** remainder returns immediately — near zero only for a fully consumed session, e.g. one left to run to `endsAt` inside one UTC day. See [Sessions: stake, close, claim](/concepts/sessions-stake-close-recover).
  </Card>
