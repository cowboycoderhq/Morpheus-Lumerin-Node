...
```mermaid
flowchart LR
  EverclawSkill[Everclaw / OpenClaw skill] --> CRouter["C-Node proxy-router (listens on http://127.0.0.1:8082)"]
...
- **Provider unreachable** — agent should retry, then the C-Node closes the stuck session and returns the remainder; the final UTC day's consumed slice day-locks until the next UTC day unless the close lands after `releaseAt`.
- **Session expired mid-task** — errors returned BY the model backend propagate their real status so impaired providers are distinguishable (`proxyapi/controller_http.go:249`), but a transport/adapter failure — including session-expired — collapses to **HTTP 500** (`:282-284`), so an agent cannot detect this from the status code. Inspect the response body, or re-check session state before retrying.
