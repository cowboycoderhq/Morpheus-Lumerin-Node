...
```bash
curl -X POST \
  'http://127.0.0.1:8082/blockchain/approve?spender=0x6aBE1d282f72B474E54527D93b979A4f64d3030a&amount=20000000000000000000' \
...
- Logs — **stdout/stderr** by default (`docker logs`, or `journalctl` for a systemd unit). There is no log *file* unless you set `LOG_FOLDER_PATH`; with it set, each run writes `<LOG_FOLDER_PATH>/<start-timestamp>/main.log` (`proxy-router/cmd/main.go:74-82`). `./data/` is the Badger store, not logs.
