...
| "I closed (or let it expire) and only part came back." | Expected — the final UTC day's slice is in the on-hold queue (Bucket 2). A running proxy-router automatically sweeps matured stake every 10 minutes for the wallet it holds, so no manual claim is needed — with the node off, or for another wallet's stake, nothing sweeps until a proxy-router holding that wallet runs: starting one claims matured stake immediately on startup, and submitting `withdrawUserStakes` yourself is the alternative. |
