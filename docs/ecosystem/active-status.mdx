...
- **Top-level** snapshot fields use PascalCase (`Id`, `Name`, `ModelName`, `PricePerSecond`, `Provider`, …). The nested objects do not: `health` and `bidDetail` are lowercase, and `bidDetail` entries use camelCase (`providerId`, `pricePerSecond`) — so a jq filter has to switch case when it descends.
