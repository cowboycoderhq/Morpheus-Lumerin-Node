...
| `STAKING_FUND_SOURCE` | `wallet` | Funding source for consumer session stakes: `wallet` (ERC-20 balance, legacy), `pool` (delegated staking pool via `openSessionFromPool`), or `auto` (wallet first, pool when the balance is insufficient). |
| `STAKE_AUTO_CLAIM` | `false` | Periodically withdraw releasable time-locked user stakes (`userStakesOnHold`) back to the wallet. Only relevant for `wallet`/`auto` funding — pool stakes recycle on-chain automatically. |
| `STAKE_AUTO_CLAIM_INTERVAL` | `1h` | How often to check for releasable stakes when `STAKE_AUTO_CLAIM` is enabled. |
