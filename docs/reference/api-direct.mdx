...
- `jq` for the shell examples below. The walkthrough pipes responses through it (`jq .address`, `jq -r '.models[]...'`, `jq -r .sessionID`). Without it the first two lines error visibly, but the session capture `SESSION=$(... | jq -r .sessionID)` fails **silently** — `SESSION` ends up empty and the following chat call returns an auth/session error instead. Install with `brew install jq` (macOS) or `apt install jq` (Debian/Ubuntu).
...
<Warning>
**`amount` is in base units, not whole tokens.** The value is passed to the ERC-20 `approve` call verbatim with no decimal scaling (`blockchainapi/service.go:1047`), and the token has 18 decimals. `amount=3` approves 3 base units — about 10⁻¹⁸ of a token — which leaves the allowance effectively zero and makes every later session-open fail. Three whole tokens is `3000000000000000000`.
</Warning>
...
curl -s -X POST "http://localhost:8082/blockchain/approve?spender=$SPENDER&amount=3000000000000000000" -H "$AUTH" -d ''
