...
- [ ] Confirm [`proxy-router/docs/swagger.yaml`](https://github.com/MorpheusAIs/Morpheus-Lumerin-Node/blob/main/proxy-router/docs/swagger.yaml) is up to date. **Regenerate it manually** — `cd proxy-router && make swagger` (`proxy-router/Makefile:57`, which runs `swag init`). CI now checks this rather than trusting it: the `spec-fresh` job in `.github/workflows/proxy-router-ci.yml:254-278` runs `scripts/check-spec-fresh.sh`, which regenerates the spec with the Makefile's own `swag init` line and fails on any difference against `docs.go`, `swagger.json` or `swagger.yaml`. Run the target here so the regenerated files land in your commit; skipping it turns the pull request red rather than shipping a stale spec. The job is guarded by `github.repository == 'cowboycoderhq/Morpheus-Lumerin-Node'`, so that holds for releases cut in this repository and for pull requests opened against it; on a fork's own branches it is skipped, and `bash scripts/check-spec-fresh.sh` is the local check to run there instead.
