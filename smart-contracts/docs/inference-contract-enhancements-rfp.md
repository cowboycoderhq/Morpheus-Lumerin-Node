...
| **Marketplace** | Providers post bids (`pricePerSecond`). Reposting auto-deletes the prior active bid for that provider↔model pair. The fee is `0.3` MOR on Base mainnet and `0` on Base Sepolia — testnet bids are free. Both are `getBidFee()` readings taken 2026-08-31 — mainnet block `50701626`, Sepolia block `46212156`, recorded in `docs/providers/full/register-onchain.mdx` under "Contract minimums" — not deploy inputs. The disagreement with the deploy inputs is therefore settled against them: `config_base_sepolia.json:6` still lists the mainnet figure, which the live Sepolia contract does not charge, because `906bb602` re-pointed `lumerinProtocol` at a different Diamond without re-running `setMarketplaceBidFee` — the sole writer of the stored fee (`Marketplace.sol:40-45`) (`docs/concepts/tokens-and-fees.mdx:37-51`). |
