...
**Intent.** Anyone can register a model, with no role gate and no human approval, but registration carries real economic weight: a refundable **bond** that makes squatting expensive and lying slashable, a small non-refundable **fee** that makes spam a toll, a **challenge** path that lets the market police truthfulness, and **idle decay** that automatically clears dead listings. This replaces the legacy free-for-all (P1/P2). A curated alternative (PR review + CI + approval multisig) was considered and rejected: it violates F2's spirit by putting humans and an automation key between a provider and the market.
