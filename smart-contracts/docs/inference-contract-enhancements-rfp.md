...
**Last updated:** 2026-06-23 19:35 UTC  
...
- **ID-R5 (aliases: name redirects, one namespace, sticky) [contract]** `registerModelAlias(sourceNameKey, targetCanonicalId)` maps a free `sourceNameKey` to an already-existing `targetCanonicalId`, so a name that is not itself a registered model resolves to another model. One mechanism serves both rebrand-forward (a legacy name redirected to the surviving model, as §3.1.5 seeding does) and synonyms (an alternate name for an existing model). Rules:
  ...
  - **Sticky, explicit-only.** An aliased name never auto-reclaims. Registering a primary model over an aliased name reverts; to free or change it the operator must first call `removeModelAlias(sourceNameKey)` or `repointModelAlias(sourceNameKey, newTargetCanonicalId)`, both operator-only and each emitting an event. A name's target therefore changes only via a loud, signed transaction, never silently (this is the P1 anti-impersonation guarantee).
  ...
  - **Name-resolution only.** Aliases never touch bid/session storage. Bids and sessions are keyed by `canonicalModelId` (ID-R1), so adding, repointing, or removing an alias leaves existing bids untouched; only what a name resolves to changes. (Subsumes the old "name to id reverse index" idea.)
  - **Reuse path.** After `removeModelAlias(bob)`, `bob` is registerable as a primary model and deterministically takes `canonicalModelId = keccak256("morpheus.model.v1/" + nameKey(bob))`, an id the alias never occupied.
...
- [ ] **AC-ID-5** `registerModelAlias` reverts when the target id does not exist or is `RETIRED`, and when `sourceNameKey` is already a primary model or an existing alias (single-namespace occupancy guard).
- [ ] **AC-ID-6** While alias `bob → ALICE` exists, `register(BOB)` reverts; after operator `removeModelAlias(bob)`, `register(BOB)` succeeds and yields `canonicalModelId == keccak256("morpheus.model.v1/" + nameKey(bob))`.
...
- [ ] **AC-ID-8** `registerModelAlias`/`repointModelAlias`/`removeModelAlias` are operator-only and emit `ModelAliasRegistered`/`ModelAliasRepointed`/`ModelAliasRemoved`; across all three, existing bids/sessions keyed by `canonicalModelId` are unchanged.
...
**Events (indexer contract):** `CanonicalModelRegistered`, `ModelMetadataUpdated`, `ModelCapabilityChanged`, `ModelLifecycleChanged`, `ModelAliasRegistered`, `ModelAliasRepointed`, `ModelAliasRemoved`.
